"""Shared-CNN PPO networks for vision + task_obs transfer learning.

Mirrors the vnl-ray pattern where a single CNN is shared between policy and
value heads, so that BOTH policy_loss and v_loss gradients flow through the
CNN.  This provides a much stronger gradient signal to the vision encoder
compared to the separate-CNN architecture in ``ppo_networks.py``.

Architecture::

        vision (H,W,C) ──> [shared VisionEncoder] ──> vision_features
                                    │
                ┌───────────────────┴───────────────────┐
                │                                       │
...
All parameters live in a single ``SharedVisionPolicyValueModule``.  In the
``PPONetworkParams`` dataclass, ``params.policy`` holds the full module
(CNN + policy head + value head) and ``params.value`` is empty.
"""
...
class SharedVisionPolicyValueModule(nn.Module):
    """Combined policy + value network with a single shared VisionEncoder.

    The CNN is instantiated once and its features are used by both the policy
    fusion path and the value MLP.  When ``jax.grad`` is computed on
    ...
    **both** losses.
    ...
    Attributes:
        ...
        vision_feature_size: Output dimension of the shared VisionEncoder.
    ...
    def __call__(
        ...
        """Full forward: shared CNN -> policy output + value output.
...
def make_shared_vision_policy(
    ...
    """
    shared_module = SharedVisionPolicyValueModule(
...
def make_shared_vision_value_stub() -> networks.FeedForwardNetwork:
    """Create a stub value network that holds no parameters.

    In the shared-CNN architecture, the value head lives inside the
    ``SharedVisionPolicyValueModule`` (i.e. inside ``params.policy``).
    ...
    def apply(processor_params, value_params, obs):
        raise RuntimeError(
            "SharedVision value stub should never be called directly. "
