...
def test_mpo_clip_params_floors_at_min():
    ...
    params = loss_fn.init_params(action_dim=6, dtype=jnp.float32)
    clipped = clip_mpo_params(params)
...
def test_mpo_clip_params_preserves_penalty_when_not_per_dim():
    """clip_mpo_params must preserve log_penalty_temperature whenever
    action_penalization=True, regardless of per_dim_constraining.

    Regression test for the bug where clip_mpo_params keyed off
    per_dim_constraining (which controls alpha shape) instead of
    action_penalization (which controls penalty existence). With the bug,
    per_dim_constraining=False + action_penalization=True silently dropped
    log_penalty_temperature (set it to None), crashing the next loss call.
    """
    loss_fn = MPO(
        epsilon=0.1,
        epsilon_mean=0.0025,
        ...
        per_dim_constraining=False,   # not per-dim
        action_penalization=True,     # but penalty IS enabled
    )
    params = loss_fn.init_params(action_dim=6, dtype=jnp.float32)
    # Pre-clip sanity: penalty temperature exists (action_penalization=True).
    assert params.log_penalty_temperature is not None

    clipped = clip_mpo_params(params)
    assert clipped.log_penalty_temperature is not None, (
        "clip_mpo_params dropped log_penalty_temperature -- "
        "would crash the next loss call when softplus(None) is computed."
    )
    assert float(clipped.log_penalty_temperature[0]) >= -18.0


def test_mpo_clip_params_no_penalty_when_action_penalization_false():
    """clip_mpo_params must return cleanly (no penalty field) when the loss
    was constructed with action_penalization=False. Covers the early-return
    branch where params.log_penalty_temperature is None.
    """
    loss_fn = MPO(
        epsilon=0.1,
        epsilon_mean=0.0025,
        epsilon_stddev=1e-7,
        epsilon_penalty=0.1,
        init_log_temperature=-100.0,     # below the -18 floor → tests the floor
        init_log_alpha_mean=-100.0,
        init_log_alpha_stddev=-100.0,
        per_dim_constraining=True,
        action_penalization=False,       # KEY: penalty disabled
    )
    params = loss_fn.init_params(action_dim=6, dtype=jnp.float32)
    # Pre-clip sanity: penalty temperature was never created.
    assert params.log_penalty_temperature is None

    clipped = clip_mpo_params(params)
    # Penalty stays None (early-return branch).
    assert clipped.log_penalty_temperature is None
    # Other dual variables are still floored at -18.
    assert float(clipped.log_temperature[0]) == -18.0
    assert float(clipped.log_alpha_mean[0]) == -18.0
    assert float(clipped.log_alpha_stddev[0]) == -18.0
