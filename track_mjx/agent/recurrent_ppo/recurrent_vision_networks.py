"""Recurrent vision PPO networks with shared CNN+GRU backbone.

This module provides a recurrent vision policy for training from egocentric
camera images using a shared CNN+GRU backbone with separate policy and value
heads. It is designed to be compatible with the recurrent PPO training
infrastructure in this package.

Architecture::

    vision (H,W,C) --> [shared VisionEncoder (CNN)] --> cnn_features
    [cnn_features, imitation_target] --> [shared GRUCell] --> gru_output
                            |
            +---------------+---------------+
            |                               |
...
All parameters live in a single ``RecurrentSharedVisionModule``. In the
``RecurrentPPONetworkParams`` dataclass, ``params.policy`` holds the full
module (CNN + GRU + policy head + value head) and ``params.value`` is empty.

Key design decisions:
- The CNN and GRU are shared between policy and value, so both losses
...
class RecurrentSharedVisionModule(nn.Module):
    """Shared CNN+GRU backbone with policy and value heads.

    The CNN encoder (VisionEncoder) processes egocentric camera images to
    produce compact feature vectors. These features are concatenated with
    the imitation target (task observation) and fed through a GRU cell for
    temporal integration. The GRU output is then routed to two heads:
    ...
    Attributes:
        ...
        cnn_feature_size: Output dimension of the VisionEncoder CNN.
        cnn_channels: Channel sizes for each conv layer in the CNN encoder.
...
def make_recurrent_vision_highlvl_ppo_networks(
    ...
    policy_hidden_sizes: Sequence[int] = (256,),
    value_hidden_sizes: Sequence[int] = (256, 128),
) -> tuple[RecurrentPPONetworks, RecurrentSharedVisionModule]:
    """Create recurrent vision PPO networks with shared CNN+GRU backbone.

    Constructs a ``RecurrentSharedVisionModule`` and wraps it as a
    ...
    Args:
        ...
        vision_shape: Spatial shape of the egocentric image (H, W, C).
        gru_hidden_size: Dimensionality of the GRU hidden state.
        cnn_feature_size: Output dimension of the VisionEncoder CNN.
        cnn_channels: Channel sizes for each conv layer in the CNN encoder.
        policy_hidden_sizes: Hidden layer sizes for the policy MLP head.
    ...
    Returns:
        Tuple of (RecurrentPPONetworks, RecurrentSharedVisionModule).
    ...
    def value_apply(processor_params, value_params, obs):
        raise RuntimeError(
            "RecurrentSharedVision value stub should never be called "
