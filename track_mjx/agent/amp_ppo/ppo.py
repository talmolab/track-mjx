...
from track_mjx.agent.amp_ppo import features, losses, networks as amp_networks
from track_mjx.agent.ff_ppo import ppo_networks
...
class TrainingState:
    """Training state for AMP PPO learner.

    Attributes:
        optimizer_state: PPO optax optimizer state.
        disc_optimizer_state: Discriminator optax optimizer state.
        ...
        disc_normalizer_params: Running statistics for AMP discriminator obs.
        replay_buffer: Fixed-size generated AMP observation replay buffer.
        replay_insert_pos: Circular replay buffer insertion position.
        replay_size: Number of valid rows in the replay buffer.
        env_steps: Total environment steps taken (in thousands).
    """

    optimizer_state: optax.OptState
    disc_optimizer_state: optax.OptState
    params: losses.PPONetworkParams
    disc_params: Params
    normalizer_params: running_statistics.RunningStatisticsState
    disc_normalizer_params: running_statistics.RunningStatisticsState
    replay_buffer: jnp.ndarray
    replay_insert_pos: jnp.ndarray
...
def train(
    ...
    if reference_clips is None:
        raise ValueError("AMP PPO requires reference_clips for demo sampling.")

    amp_config = {} if amp_config is None else dict(amp_config)
    disc_hidden_layer_sizes = tuple(amp_config.get("disc_layer_sizes", (1024, 1024)))
    disc_learning_rate = amp_config.get("disc_learning_rate", 2.5e-4)
    disc_weight_decay = amp_config.get("disc_weight_decay", 1e-4)
    disc_epochs = int(amp_config.get("disc_epochs", 2))
    disc_batch_size = int(amp_config.get("disc_batch_size", batch_size))
    disc_replay_size = int(amp_config.get("disc_replay_size", 200_000))
    disc_replay_samples = int(amp_config.get("disc_replay_samples", 1000))
    disc_logit_reg = amp_config.get("disc_logit_reg", 0.01)
    disc_grad_penalty = amp_config.get("disc_grad_penalty", 5.0)
    disc_reward_scale = amp_config.get("disc_reward_scale", 2.0)
    task_reward_weight = amp_config.get("task_reward_weight", 0.0)
    disc_reward_weight = amp_config.get("disc_reward_weight", 1.0)
    disc_obs_clip = amp_config.get("disc_obs_clip", 10.0)
    disc_grad_clip_threshold = amp_config.get(
        "disc_grad_clip_threshold", grad_clip_threshold
    )
    num_disc_obs_steps = int(amp_config.get("num_disc_obs_steps", 10))
    configured_key_bodies = amp_config.get("key_bodies", amp_key_body_names)
    amp_key_body_names = (
        tuple(configured_key_bodies) if configured_key_bodies is not None else ()
    )
    if disc_replay_size <= 0:
        raise ValueError("amp_config.disc_replay_size must be positive.")
    if disc_batch_size <= 0:
        raise ValueError("amp_config.disc_batch_size must be positive.")
    ...
    def _flatten_amp_obs(data: types.Transition) -> jnp.ndarray:
        amp_obs = data.extras["state_extras"]["amp_obs"]
        return amp_obs.reshape((-1, amp_obs.shape[-1]))

    def _sample_demo_amp_obs(key: PRNGKey, num_samples: int) -> jnp.ndarray:
        return features.sample_reference_amp_obs(
            reference_clips=reference_clips,
            key=key,
            num_samples=num_samples,
            num_disc_obs_steps=num_disc_obs_steps,
    ...
    def _update_replay(
        ...
        insert_count = min(
            amp_obs.shape[0],
            disc_replay_samples if disc_replay_samples > 0 else amp_obs.shape[0],
        )
        perm = jax.random.permutation(key, amp_obs.shape[0])
        insert_obs = amp_obs[perm[:insert_count]]
        insert_idx = (
            replay_insert_pos + jnp.arange(insert_count, dtype=jnp.int32)
        ) % replay_buffer.shape[0]
        replay_buffer = replay_buffer.at[insert_idx].set(insert_obs)
        replay_insert_pos = (replay_insert_pos + insert_count) % replay_buffer.shape[0]
        replay_size = jnp.minimum(replay_buffer.shape[0], replay_size + insert_count)
        return replay_buffer, replay_insert_pos, replay_size
    ...
    def _sample_replay(
        ...
        valid_size = jnp.maximum(replay_size, 1)
        idx = jax.random.randint(key, (sample_count,), 0, valid_size)
        return replay_buffer[idx]
    ...
    def _update_discriminator(
        ...
        demo_obs: jnp.ndarray,
        replay_buffer: jnp.ndarray,
        replay_size: jnp.ndarray,
        key: PRNGKey,
    ) -> tuple[optax.OptState, Params, Metrics]:
        actual_batch_size = min(disc_batch_size, agent_obs.shape[0])
        num_disc_batches = int(np.ceil(agent_obs.shape[0] / actual_batch_size))
        num_disc_steps = max(1, disc_epochs * num_disc_batches)

        def disc_step(carry, unused_t):
            disc_optimizer_state, disc_params, key = carry
            key, key_agent, key_demo, key_replay = jax.random.split(key, 4)

            agent_idx = jax.random.randint(
                key_agent, (actual_batch_size,), 0, agent_obs.shape[0]
            )
            demo_idx = jax.random.randint(
                key_demo, (actual_batch_size,), 0, demo_obs.shape[0]
            )
            replay_obs = _sample_replay(
                replay_buffer, replay_size, actual_batch_size, key_replay
            )

            (_, metrics), disc_params, disc_optimizer_state = disc_gradient_update_fn(
                disc_params,
                disc_normalizer_params,
                agent_obs[agent_idx],
                demo_obs[demo_idx],
                replay_obs,
                optimizer_state=disc_optimizer_state,
                params=disc_params,
            )
            return (disc_optimizer_state, disc_params, key), metrics

        (disc_optimizer_state, disc_params, _), metrics = jax.lax.scan(
            disc_step,
            (disc_optimizer_state, disc_params, key),
            (),
            length=num_disc_steps,
        )
        metrics = jax.tree_util.tree_map(jnp.mean, metrics)
        return disc_optimizer_state, disc_params, metrics
    ...
    def training_step(
        ...
        def f(carry, unused_t):
            ...
            next_state, data = acting.generate_unroll(
                ...
                current_key,
                unroll_length,
                extra_fields=("truncation", "amp_obs"),
            )
        ...
        flat_amp_obs = _flatten_amp_obs(data)
        flat_demo_amp_obs = _sample_demo_amp_obs(key_demo, flat_amp_obs.shape[0])
        disc_normalizer_params = running_statistics.update(
            training_state.disc_normalizer_params,
            jnp.concatenate([flat_amp_obs, flat_demo_amp_obs], axis=0),
            pmap_axis_name=_PMAP_AXIS_NAME,
        )

        data, reward_metrics = losses.replace_rewards_with_amp(
            data,
            disc_params=training_state.disc_params,
            disc_normalizer_params=disc_normalizer_params,
            disc_network=disc_network,
            task_reward_weight=task_reward_weight,
            disc_reward_weight=disc_reward_weight,
            disc_reward_scale=disc_reward_scale,
            disc_obs_clip=disc_obs_clip,
        )

        replay_buffer, replay_insert_pos, replay_size = _update_replay(
            training_state.replay_buffer,
            training_state.replay_insert_pos,
            training_state.replay_size,
            flat_amp_obs,
            key_replay,
        )

        (optimizer_state, params, _, _), ppo_metrics = jax.lax.scan(
        ...
        disc_optimizer_state, disc_params, disc_metrics = _update_discriminator(
            training_state.disc_optimizer_state,
            training_state.disc_params,
            disc_normalizer_params,
            flat_amp_obs,
            flat_demo_amp_obs,
            replay_buffer,
        ...
        metrics = {
            ...
            **disc_metrics,
            **reward_metrics,
            "disc_replay_size": replay_size,
        }

        new_training_state = TrainingState(
            optimizer_state=optimizer_state,
            disc_optimizer_state=disc_optimizer_state,
            params=params,
            disc_params=disc_params,
            normalizer_params=normalizer_params,
            disc_normalizer_params=disc_normalizer_params,
            replay_buffer=replay_buffer,
            replay_insert_pos=replay_insert_pos,
    ...
    key_policy, key_value, key_disc, policy_params_fn_key = jax.random.split(
        global_key, 4
    )
    del global_key
    ...
    amp_obs_size = int(env_state.info["amp_obs"].shape[-1])
    logging.info(f"AMP observation size: {amp_obs_size}")
    config_dict.setdefault("amp_config", {}).update(
        {
            "amp_obs_size": amp_obs_size,
            "key_bodies": list(amp_key_body_names),
            "num_disc_obs_steps": num_disc_obs_steps,
        }
    )
    disc_network = amp_networks.make_amp_discriminator_network(
        obs_size=amp_obs_size,
        hidden_layer_sizes=disc_hidden_layer_sizes,
        use_layer_norm=amp_config.get("disc_layer_norm", False),
    )
    ...
    disc_optimizer = optax.chain(
        optax.clip_by_global_norm(disc_grad_clip_threshold),
        optax.adamw(
    ...
    init_disc_params = disc_network.init(key_disc)

    # Initialize normalizer with pytree structure matching observations
    obs_shape = get_obs_shape(env_state.obs)
    training_state = TrainingState(
        optimizer_state=optimizer.init(init_params),
        disc_optimizer_state=disc_optimizer.init(init_disc_params),
        params=init_params,
        disc_params=init_disc_params,
        normalizer_params=running_statistics.init_state(obs_shape),
        disc_normalizer_params=running_statistics.init_state(
            specs.Array((amp_obs_size,), jnp.dtype("float32"))
        ),
        replay_buffer=jnp.zeros((disc_replay_size, amp_obs_size), dtype=jnp.float32),
        replay_insert_pos=jnp.array(0, dtype=jnp.int32),
        replay_size=jnp.array(0, dtype=jnp.int32),
        env_steps=0,
    )
    ...
    disc_loss_fn = functools.partial(
        losses.compute_discriminator_loss,
        disc_network=disc_network,
        disc_grad_penalty=disc_grad_penalty,
        disc_logit_reg=disc_logit_reg,
        disc_obs_clip=disc_obs_clip,
    )
    disc_gradient_update_fn = gradients.gradient_update_fn(
        disc_loss_fn,
        disc_optimizer,
        pmap_axis_name=_PMAP_AXIS_NAME,
        has_aux=True,
