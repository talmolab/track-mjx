...
def update(
    ...
    *,
    ...
    Arguments:
      ...
      mask: Boolean mask, where True is the dims that you want to update.
    ...
    # Don't update some dims if a boolean mask is given
    if mask is not None:
        mean = jnp.where(mask, mean, state.mean)
        std = jnp.where(mask, std, state.std)
        summed_variance = jnp.where(mask, summed_variance, state.summed_variance)
...
def denormalize(batch: types.NestedArray, mean_std: NestedMeanStd) -> types.NestedArray:
