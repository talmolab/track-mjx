...
def rollout_logging_fn(
    ...
    max_frames: int = None,  # New parameter to control video trim length in logging render
) -> None:
    ...
    Args:
        ...
        max_frames: Optional parameter to control video trim length in logging render.
    """
    ...
    # Compute downsample rate (sim_fps = 1/env.dt, desired fps = cfg.env_config.render_fps)
    desired_fps = cfg.env_config.render_fps
    sim_fps = 1.0 / env.dt
    downsample_rate = int(sim_fps / desired_fps)
    # Get step info from rollout states to detect resets
