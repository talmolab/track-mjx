"""Generic observation utilities for maintain_velocity task.

These utilities handle arbitrary observation dict keys.
"""
...
def flatten_obs_dict(obs: Mapping[str, Any]) -> dict[str, jnp.ndarray]:
    ...
    Generic version that handles ANY keys in the observation dict.
...
def concat_flat_dict_obs(obs: Mapping[str, jnp.ndarray]) -> jnp.ndarray:
    ...
    For maintain_velocity: [task_obs | proprioception]
