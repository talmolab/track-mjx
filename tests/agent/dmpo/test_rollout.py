...
def test_collect_rollout_shapes():
    ...
    def policy_fn(obs, key):
        return jax.random.normal(key, (env.action_size,))

    traj, _ = collect_rollout(env, policy_fn, rng, num_envs=8, num_steps=4)
...
def test_collect_rollout_stores_raw_pretanh_action():
    ...
    def policy_fn(obs, key):
        return LARGE * jnp.ones((env.action_size,))

    traj, _ = collect_rollout(env, policy_fn, rng, num_envs=2, num_steps=3)
...
def test_collect_rollout_is_jittable():
    ...
    @jax.jit
    def go(rng):
        return collect_rollout(env, policy_fn, rng, num_envs=4, num_steps=3)
