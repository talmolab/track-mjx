...
class RecurrentDecoder(nn.Module):
    ...
    def __call__(
    ...
    ) -> tuple[jnp.ndarray, list[HiddenState]] | tuple[jnp.ndarray, list[HiddenState], dict]:
        ...
        Args:
            ...
            get_activation: If True, return activations dict as third element.

        Returns:
            If get_activation is False: (output, new_hidden_states).
            If get_activation is True: (output, new_hidden_states, activations).
        """
        ...
        new_hidden_list = []
        rnn_input = x
        activations = {} if get_activation else None
        ...
        for i, (cell, h) in enumerate(zip(self.rnn_cells, hidden_list)):
            ...
            if self.cell_type == "lstm":
                ...
                rnn_input = new_h
            if get_activation:
                activations[f"layer_{i}"] = rnn_input
...
class RecurrentIntentionNetwork(nn.Module):
    ...
    def __call__(
        ...
        Args:
            ...
            get_activation: If True, return activations dict as fifth element.

        Returns:
            If get_activation is False:
                (action_params, latent_mean, latent_logvar, new_hidden)
            If get_activation is True:
