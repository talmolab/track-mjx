...
def normalize_disc_obs(
    normalizer_params: Any,
    disc_obs: jnp.ndarray,
    clip: float = 10.0,
) -> jnp.ndarray:
    """Normalize discriminator observations with optional clipping."""
    normalized = (disc_obs - normalizer_params.mean) / normalizer_params.std
    return jnp.clip(normalized, -clip, clip)


def compute_disc_rewards(
    disc_params: Params,
    disc_normalizer_params: Any,
    disc_obs: jnp.ndarray,
    disc_network: Any,
    disc_reward_scale: float = 2.0,
    disc_obs_clip: float = 10.0,
) -> jnp.ndarray:
    """Compute AMP discriminator rewards from generated motion observations."""
    norm_disc_obs = normalize_disc_obs(
        disc_normalizer_params, disc_obs, clip=disc_obs_clip
    )
    logits = disc_network.apply(disc_params, norm_disc_obs)
    prob = jax.nn.sigmoid(logits)
    reward = -jnp.log(jnp.maximum(1.0 - prob, 1e-4))
    return reward * disc_reward_scale


def replace_rewards_with_amp(
    ...
    disc_reward_scale: float = 2.0,
    disc_obs_clip: float = 10.0,
) -> tuple[types.Transition, types.Metrics]:
    """Replace environment rewards with AMP-weighted rewards for PPO."""
    amp_obs = data.extras["state_extras"]["amp_obs"]
    flat_amp_obs = amp_obs.reshape((-1, amp_obs.shape[-1]))
    flat_disc_rewards = compute_disc_rewards(
        disc_params=disc_params,
        disc_normalizer_params=disc_normalizer_params,
        disc_obs=flat_amp_obs,
        disc_network=disc_network,
        disc_reward_scale=disc_reward_scale,
        disc_obs_clip=disc_obs_clip,
    )
    disc_rewards = flat_disc_rewards.reshape(data.reward.shape)
    combined_reward = (
        task_reward_weight * data.reward + disc_reward_weight * disc_rewards
    )
    combined_reward = jax.lax.stop_gradient(combined_reward)

    return data._replace(reward=combined_reward), {
        "amp_disc_reward_mean": jnp.mean(disc_rewards),
        "amp_disc_reward_std": jnp.std(disc_rewards),
        "amp_task_reward_mean": jnp.mean(data.reward),
        "amp_combined_reward_mean": jnp.mean(combined_reward),
    }


def _logit_weight_l2(disc_params: Params) -> jnp.ndarray:
    """Return the squared norm of the discriminator output-layer weights."""
    params = disc_params.get("params", disc_params)
    logits = params.get("logits", {})
    kernel = logits.get("kernel")
    if kernel is None:
        return jnp.array(0.0)
    return jnp.sum(jnp.square(kernel))
...
def compute_discriminator_loss(
    ...
    replay_obs: jnp.ndarray,
    disc_network: Any,
    disc_grad_penalty: float = 5.0,
    disc_logit_reg: float = 0.01,
    disc_obs_clip: float = 10.0,
) -> tuple[jnp.ndarray, types.Metrics]:
    """Compute AMP discriminator binary classification loss."""
    agent_obs = jnp.concatenate([agent_obs, replay_obs], axis=0)
    norm_agent_obs = normalize_disc_obs(
        disc_normalizer_params, agent_obs, clip=disc_obs_clip
    )
    norm_demo_obs = normalize_disc_obs(
        disc_normalizer_params, demo_obs, clip=disc_obs_clip
    )

    agent_logits = disc_network.apply(disc_params, norm_agent_obs)
    demo_logits = disc_network.apply(disc_params, norm_demo_obs)

    agent_loss = jnp.mean(
        optax.sigmoid_binary_cross_entropy(agent_logits, jnp.zeros_like(agent_logits))
    )
    demo_loss = jnp.mean(
        optax.sigmoid_binary_cross_entropy(demo_logits, jnp.ones_like(demo_logits))
    )
    disc_loss = 0.5 * (agent_loss + demo_loss)

    def logit_sum(obs):
        return jnp.sum(disc_network.apply(disc_params, obs))

    agent_grad = jax.grad(logit_sum)(norm_agent_obs)
    demo_grad = jax.grad(logit_sum)(norm_demo_obs)
    grad_penalty = 0.5 * (
        jnp.mean(jnp.sum(jnp.square(agent_grad), axis=-1))
        + jnp.mean(jnp.sum(jnp.square(demo_grad), axis=-1))
    )
    logit_loss = _logit_weight_l2(disc_params)

    total_loss = (
        disc_loss + disc_grad_penalty * grad_penalty + disc_logit_reg * logit_loss
    )
    return total_loss, {
        "disc_loss": total_loss,
        "disc_bce_loss": disc_loss,
        "disc_agent_loss": agent_loss,
        "disc_demo_loss": demo_loss,
        "disc_grad_penalty": grad_penalty,
        "disc_logit_loss": logit_loss,
        "disc_agent_acc": jnp.mean((agent_logits < 0.0).astype(jnp.float32)),
        "disc_demo_acc": jnp.mean((demo_logits > 0.0).astype(jnp.float32)),
        "disc_agent_logit": jnp.mean(agent_logits),
        "disc_demo_logit": jnp.mean(demo_logits),
    }
