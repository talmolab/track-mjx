...
from track_mjx.agent import acting
...
def train(
    ...
    jit_logging_inference_fn = jax.jit(make_logging_policy(
        deterministic=True,
    ))
    ...
    def sgd_step(
        ...
        def convert_data(x: jnp.ndarray):
            x = jax.random.permutation(key_perm, x) # this shuffles along the first axis (16384), I think it is fine to do this even with LSTM
            x = jnp.reshape(x, (num_minibatches, -1) + x.shape[1:])
            return x # (4, 4096, 20, 617) or (4, 4096, 20) or (4, 4096, 20, 60) or (4, 4096, 20, 76) or (4, 4096, 20, 38)
        # data (16384, 20, 617) # ?, unroll_length, ?
        # shuffled data (4, 4096, 20, 617)
        # num_minibatches = 4
        # batch_size = 4096
    ...
    def training_step(
        ...
        # data.observation.shape = (2, 20, 8192, 617)
        data = jax.tree_util.tree_map(lambda x: jnp.swapaxes(x, 1, 2), data) # (2, 8192, 20, 617)
    ...
    def training_epoch(
        ...
        (training_state, state, _, _), loss_metrics = jax.lax.scan( # state.obs.shape = (8192, 617)
            training_step,
