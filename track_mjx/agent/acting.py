...
def actor_step(
  ...
  """Collect data."""
  actions, policy_extras = policy(env_state.obs, key) # actions.shape=(128, 38), env_state.obs.shape=(128, 617), now it is (8192, 617)
...
def actor_step_v2(
  ...
  """Collect data."""
  actions, policy_extras = policy(env_state.obs, key) # policy_extras has mu_z and logvar_z
  nstate = env.step(env_state, actions)
  state_extras = {x: nstate.info[x] for x in extra_fields}
  return nstate, Transition(  # pytype: disable=wrong-arg-types  # jax-ndarray
      observation=env_state.obs,
...
def generate_unroll_v2(
  ...
  (final_state, _), data = jax.lax.scan(
      f, (env_state, key), (), length=unroll_length
  ) # should be output of LSTM model
  return final_state, data
